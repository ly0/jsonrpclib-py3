...
class JSONTarget(object):
    ...
    def close(self):
        return ''.join(map(lambda x: x.decode(), self.data))
...
class ServerProxy(XMLServerProxy):
    ...
    def __init__(self, uri, transport=None, encoding=None, 
        ...
        schema, uri = urllib.parse.splittype(uri)
...
class MultiCallMethod(object):
    ...
    def request(self, encoding=None, rpcid=None):
        return dumps(self.params, self.method, version=2.0,
                      rpcid=rpcid, notify=self.notify)
...
class Payload(dict):
    ...
    def request(self, method, params=[]):
        if type(method) not in [str, bytes]:
...
def dumps(params=[], methodname=None, methodresponse=None, 
    ...
    if not version:
        version = config.version
    valid_params = (list, tuple, dict)
    if methodname in [str, bytes] and \
    ...
    if type(methodname) not in [str, bytes] and methodresponse != True:
